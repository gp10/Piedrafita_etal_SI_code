function [nx_basal,nx_total,ntime,X,Tini,Tend,AllNoDiv] = MonteCarloSimulator_2xSC_TotalCloneDynamics_CellProlif(rtime,labelS1frac,lambdaS1,lambdaS2,uS1,gammaS1,gammaS2,mu,indiv,tlagDivS1,GamShapeDivS1,tlagStrS1,GamShapeStrS1,tlagDivS2,GamShapeDivS2,tlagStrS2,GamShapeStrS2,tlagShed,GamShapeShed)
%% Non-Markovian Monte Carlo simulator of 2 Independent Stem Cell (2xSC) model dynamics (with historical record of cell type and proliferation status)
...
% X: cell array of {1,m} shape, storing individual cell types in each of the m clones
% Tini: cell array of {1,m} shape, storing individual cell time of birth in each of the m clones
% Tend: cell array of {1,m} shape, storing individual cell time of division/stratification in each of the m clones
% AllNoDiv: cell array of {1,m} shape, storing the past number of division rounds experienced by each individual cell during the course of the simulation
...
% tlagShed = 0;
% GamShapeShed = 2;
%[nx_basal,nx_total,ntime,X,Tini,Tend,AllNoDiv] = MonteCarloSimulator_2xSC_TotalCloneDynamics_CellProlif(rtime,labelS1frac,lambdaS1,lambdaS2,uS1,gammaS1,gammaS2,mu,indiv,tlagDivS1,GamShapeDivS1,tlagStrS1,GamShapeStrS1,tlagDivS2,GamShapeDivS2,tlagStrS2,GamShapeStrS2,tlagShed,GamShapeShed);
        ...
        if tini(Id_Cell,1) <= timelim
            ...
            if (event==1)
                ...
                if tend(Id_Cell,1) < timelim
                    ...
                    NoDiv = [NoDiv; NoDiv(Id_Cell,1)+1; NoDiv(Id_Cell,1)+1];
                end
            elseif (event==2)
                if Id_Cell==1 % if founder type-1 stem cell
