function [OK_lambda,OK_tlag,OK_GamShape] = ABCrejection_tccDist(rtime,log2n_H2BGFP_Int_all,N,TolThres,lambda_avg,tlag_range,GamShape_range)
%% Approximate Bayesian Computation (ABC) algorithm to find best-fitting cell-cycle parameters for H2BGFP dilution pattern
...
% from Monte-Carlo simulations of basal-cell turnover).

% from Piedrafita et al, 2018

%% Input:
% rtime: horizontal vector containing desired time points (expressed in weeks)
% log2n_H2BGFP_Int_all: {1,n} array containing experimental distributions of H2BGFP intensities for the different time points (expressed as log2 values; column vectors)
...
%% Example:
% rtime = [0 1 2 3]; %(weeks)
% log2n_H2BGFP_Int_all{1,1} = normrnd(0,0.2,1000,1); ...
%       log2n_H2BGFP_Int_all{1,2} = normrnd(-1,0.2,1000,1); ...
%       log2n_H2BGFP_Int_all{1,3} = normrnd(-2,0.2,1000,1); ...
%       log2n_H2BGFP_Int_all{1,4} = normrnd(-3,0.2,1000,1);
% N = 100;
% TolThres = 0.15;
% lambda_avg = 1;
% tlag_range = [0:0.25:2]./7;
% GamShape_range = 2.^[0:6]; %[1,2,4,8,16,32,64];
% [OK_lambda,OK_tlag,OK_GamShape] = ABCrejection_tccDist(rtime,log2n_H2BGFP_Int_all,N,TolThres,lambda_avg,tlag_range,GamShape_range);
...
% Experimental H2BGFP intensity values are normalized to avg at time0
rlog2_Idist = {};
...
for aja = 1:N
    ...
    while pdist < TolThres
        ...
        % Draw random parameter values from prior distributions:
        lambda = lambda_avg; %2.8+rand*1.2;
        tlag = rand*(tlag_range(end)-tlag_range(1)) + tlag_range(1); % up to 2d maximum
